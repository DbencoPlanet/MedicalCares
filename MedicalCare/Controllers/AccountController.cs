        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (await _signInManager.UserManager.IsInRoleAsync(user, "Admin") || await _signInManager.UserManager.IsInRoleAsync(user, "SuperAdmin"))
                    {
                        return RedirectToAction("Index", "DashBoard", new { area = "Admin" });
                    }
                    else if (await _signInManager.UserManager.IsInRoleAsync(user, "Doctor"))
                    {
                        return RedirectToAction("Index", "DashBoard", new { area = "Doctor" });
                    }
                    else if (await _signInManager.UserManager.IsInRoleAsync(user, "Patient"))
                    {
                        return RedirectToAction("Index", "DashBoard", new { area = "Patient" });
                    }
                    else if (await _signInManager.UserManager.IsInRoleAsync(user, "Nurse"))
                    {
                        return RedirectToAction("Index", "DashBoard", new { area = "Nurse" });
                    }
                    else if (await _signInManager.UserManager.IsInRoleAsync(user, "Pharmacist"))
                    {
                    ...
                    else if (await _signInManager.UserManager.IsInRoleAsync(user, "Laboratorist"))
                    {
                        return RedirectToAction("Index", "DashBoard", new { area = "Laboratorist" });
                    }

                    else if (await _signInManager.UserManager.IsInRoleAsync(user, "Accountant"))
                    {
