...
namespace MedicalCare.Areas.Admin.Data.Services
{
    public class PharmacistService : IPharmacistService
    {
        ...
        public PharmacistService(UserManager<ApplicationUser> userManager,
        ...
        public async Task EditPharmacistAsync(PharmacistDto models, List<IFormFile> files)
        {
            var fileName = await _dotnetdesk.UploadFile(files, _env);

            Pharmacist pha = await _context.Pharmacists.FindAsync(models.Id);
        ...
        public async Task<Pharmacist> GetPharmacistWithoutIdAsync()
        {
            var user = _contextAccessor.HttpContext.User?.Identity?.Name;

            var pha = await _context.Pharmacists.Include(x => x.User).FirstOrDefaultAsync(x => x.UserId == user);
            return pha;
        }

        public async Task<List<Pharmacist>> ListPharmacistAsync()
        {
            var items = await _context.Pharmacists.Include(x => x.User).Include(x => x.User).ToListAsync();
            return items;
        }
    }
}
