...
{
    public class AccountantsService : IAccountantsService
    {
        ...
        public AccountantsService(UserManager<ApplicationUser> userManager,
        ...
        public async Task EditAccountantAsync(AccountantDto models, List<IFormFile> files)
        {
        ...
        {
            var pat = await _context.Accountants.Include(x => x.User).FirstOrDefaultAsync(x => x.Id == id);
            return pat;
        }

        public async Task<Accountants> GetAccountantWithoutIdAsync()
        {
            var user = _contextAccessor.HttpContext.User?.Identity?.Name;

            var pat = await _context.Accountants.Include(x => x.User).FirstOrDefaultAsync(x => x.UserId == user);
            return pat;
        }

        public async Task<List<Accountants>> ListAccountantAsync()
        {
            var items = await _context.Accountants.Include(x => x.User).Include(x => x.User).ToListAsync();
            return items;
        }
    }
}
