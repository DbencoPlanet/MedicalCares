...
namespace MedicalCare.Areas.Admin.Data.Services
{
    public class NurseService : INurseService
    {
        ...
        public NurseService(UserManager<ApplicationUser> userManager,
        ...
        public async Task EditNurseAsync(NurseDto models, List<IFormFile> files)
        {
            var fileName = await _dotnetdesk.UploadFile(files, _env);

            Nurse pha = await _context.Nurse.FindAsync(models.Id);
        ...
        {
            var pha = await _context.Nurse.Include(x => x.User).FirstOrDefaultAsync(x => x.Id == id);
            return pha;
        }

        public async Task<Nurse> GetNurseWithoutIdAsync()
        {
            var user = _contextAccessor.HttpContext.User?.Identity?.Name;

            var pha = await _context.Nurse.Include(x => x.User).FirstOrDefaultAsync(x => x.UserId == user);
            return pha;
        }

        public async Task<List<Nurse>> ListNurseAsync()
        {
            var items = await _context.Nurse.Include(x => x.User).Include(x => x.User).ToListAsync();
            return items;
        }
    }
}
